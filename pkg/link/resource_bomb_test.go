...
func TestBeginIncomingResource_RejectsTransferSizeBeyondMaxEfficient(t *testing.T) {
	l := &Link{}
	l.mdu = 384
	...
	adv := &resource.ResourceAdvertisement{
		...
		DataSize:     1024,
		RandomHash:   bytes.Repeat([]byte{0x11}, resource.RandomHashSize),
		Hash:         bytes.Repeat([]byte{0x12}, 32),
		Hashmap:      makeFakeHashmap(1),
	}

	if err := l.beginIncomingResource(adv); err == nil ||
		!strings.Contains(err.Error(), "MaxEfficientSize") {
		t.Fatalf("expected MaxEfficientSize rejection, got: %v", err)
	}
}

func TestBeginIncomingResource_RejectsPartsBeyondSegmentCapacity(t *testing.T) {
	l := &Link{}
	l.mdu = 384
	sdu := l.resourceSDU()
	if sdu <= 0 {
	...
	tooMany := int((int64(resource.MaxEfficientSize)+4096)/int64(sdu)) + 9

	adv := &resource.ResourceAdvertisement{
		Parts:        tooMany,
		TransferSize: 1024,
		DataSize:     1024,
		RandomHash:   bytes.Repeat([]byte{0x13}, resource.RandomHashSize),
		Hash:         bytes.Repeat([]byte{0x14}, 32),
		Hashmap:      makeFakeHashmap(1),
	}

	if err := l.beginIncomingResource(adv); err == nil ||
		!strings.Contains(err.Error(), "MaxSegments") {
		t.Fatalf("expected segment parts rejection, got: %v", err)
	}
}
