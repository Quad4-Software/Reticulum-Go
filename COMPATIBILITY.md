...
For crypto and storage see [docs/en/cryptography.md](docs/en/cryptography.md). For behavior and threat model see [SECURITY.md](SECURITY.md). For intentional differences from upstream see [docs/en/deviations.md](docs/en/deviations.md). Full English docs: [docs/en/](docs/en/README.md).

## Interop methodology

Compatibility is verified against the current reference implementation, not just self-round-trips:

- **Byte-level crossref**: `tests/crossref/` clones Python RNS via rngit and compares generated bytes (packets, announces, links, identities, proofs, msgpack layouts) field-for-field against the reference.
- **Live interop**: `tests/interop/*_live_test.go` runs real links between this stack and a pinned `rns==1.5.4` Python venv in both directions (Go client to Python server and Python client to Go server) over real transports: TCP client/server, UDP, serial over PTY, KISS, pipe via TCP bridge, WebSocket, WebTransport, and IFAC-sealed paths. The `test-live-interop` CI job runs this suite on every push.
- **Utility parity**: rnstatus/rnprobe/rncp/rnsh/rnid equivalents are exercised against Python counterparts (`rgoprobe`/`rnprobe` both directions, `rncp` sender to `rgocp` listener, `rgospeed`/`rgosnap` pairs, Python `rnsh` sessions).
- **LXMF stamps**: outbound stamp generation is validated byte-for-byte against upstream `LXStamper` fixtures.
- **Interop timeline**: both sides emit the shared `INTEROP_EVENT` JSONL format ([docs/en/interop-timeline.md](docs/en/interop-timeline.md)) so peer behavior can be diffed event-by-event.
...
| Hardening bounds | In-memory caps on attacker-influenced state upstream leaves unbounded: split-resource assemblies (4 per link, 1 GiB each, sequential segments only), RNode packet queues (256 per direction), RawChannelReader unread buffer (8 MiB). Wire format unchanged. See [docs/en/deviations.md](docs/en/deviations.md). |
