...
| RNodeInterface | Yes | [rnode.go](pkg/interfaces/rnode.go), KISS framing in [rnode_kiss.go](pkg/interfaces/rnode_kiss.go), and simulator coverage in [rnode_sim.go](pkg/interfaces/rnode_sim.go). Serial and TCP transports on Linux (including Android GOOS), macOS, Windows, FreeBSD, and OpenBSD. NetBSD/Dragonfly build the driver with TCP and injectable Open (native TTY via go.bug.st/serial is incomplete there). `ble://`, `bt://`, and `usb://` open via [rnode_transport.go](pkg/interfaces/rnode_transport.go) host registration (`RegisterRNodePortOpener`) or Android helpers under [bindings/android](bindings/android) bridged as `tcp://127.0.0.1:port`. |
| RNodeMultiInterface | Yes | [rnode_multi.go](pkg/interfaces/rnode_multi.go). Same platform coverage as RNodeInterface. Virtual-port discovery, child registration, selected-radio configuration and transmit, indexed receive delivery, flow control, and firmware validation. |
...
| Android KISS / RNode / Serial | Yes | RNode/Serial compile for `GOOS=android`. USB Host bulk serial, Nordic UART BLE, and classic RFCOMM helpers live in [bindings/android](bindings/android) (usb4a/`able`-style). Bridge with `RNodeTcpBridge` + Go `tcp://127.0.0.1:port`, or `RegisterRNodePortOpener` + [rnode_hostpipe.go](pkg/interfaces/rnode_hostpipe.go) for JNI/FFI. |
