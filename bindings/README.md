...
| **Python RNS** | Official `rns` (1.3.9) / NomadNet-style peers | Live announce, path, link, request, resource, or pageserver against Python helpers in `tests/interop/py/`. Crossref vectors in `tests/crossref/` for byte-level crypto and packet parity |
